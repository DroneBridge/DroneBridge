...
db_video_rx_t *db_video_rx_memory_open(void) {
    int fd;
    for(;;) {
        fd = shm_open("/db_video_rx_t", O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
        if(fd > 0) {
            break;
        }
    ...
    if (ftruncate(fd, sizeof(db_video_rx_t)) == -1) {
        perror("db_video_rx_memory_open: ftruncate");
        exit(1);
    }

    void *retval = mmap(NULL, sizeof(db_video_rx_t), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    if (retval == MAP_FAILED) {
        perror("db_video_rx_memory_open: mmap");
        exit(1);
    }
    return (db_video_rx_t*)retval;
}

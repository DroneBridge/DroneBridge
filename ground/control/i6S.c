...
int initialize_i6S(int new_Joy_IF){
    ...
    printf("Calibrating...\n");
    int returnval = system(i6S_CALIBRATION);
    if(returnval>=0){
...
//    double begin = (tv.tv_sec) * 1000 + (tv.tv_usec) / 1000;
    ...
    while( keepRunning ) //send loop
    {
        ...
        // Arm switch
        if(rc.button0==1)
        {rc.button0=1000;} else if(rc.button0==0) {rc.button0=2000;}
        ...
        // failsafe
        if(rc.button5==0)
        {rc.button5=1000;}else if(rc.button5==1){rc.button5=2000;}
        ...
        // Check if there is input from the socket (smartphone could send some extra commands)
//        printf( "%c[;H", 27 );
...
//                printf("Dauer für 120 mal senden: %f ms",end-begin);
//                keepRunning = false;
//            }
    }
    close(fd);

...
#	echo "NICS: $NICS"
	...
	if [ "$TXMODE" != "single" ]; then
		...
		/root/wifibroadcast/rx -p 0 -d 1 -t 6 -b $VIDEO_BLOCKS -r $VIDEO_FECS -f $VIDEOBLOCKLENGTH $NICS >/dev/null &
		sleep 0.5
		echo
...
function tx_function {
    ...
    if vcgencmd get_throttled | nice grep -q -v "0x0"; then
        ...
        collect_debug
    nice -n -9 raspivid -w $WIDTH -h $HEIGHT -fps $FPS -b 3000000 -g $KEYFRAMERATE -t 0 $EXTRAPARAMS -ae 40,0x00,0x8080FF -a "\n\nunder-voltage or over-temperature on TX!" -o - | nice -n -9 /root/wifibroadcast/tx -p 0 -b $VIDEO_BLOCKS -r $VIDEO_FECS -f $VIDEO_BLOCKLENGTH -t $VIDEO_FRAMETYPE -d $VIDEO_WIFI_BITRATE -y 0 $NICS
    sleep 365d
    fi
    ...
    nice -n -9 raspivid -w $WIDTH -h $HEIGHT -fps $FPS -b $BITRATE -g $KEYFRAMERATE -t 0 $EXTRAPARAMS $ANNOTATION -o - | nice -n -9 /root/wifibroadcast/tx -p 0 -b $VIDEO_BLOCKS -r $VIDEO_FECS -f $VIDEO_BLOCKLENGTH -t $VIDEO_FRAMETYPE -d $VIDEO_WIFI_BITRATE -y 0 $NICS
...
function rx_function {
	...
	if [ "$RELAY" == "Y" ]; then
	    ionice -c 1 -n 4 nice -n -10 cat /root/videofifo4 | /root/wifibroadcast/tx -p 0 -b $RELAY_VIDEO_BLOCKS -r $RELAY_VIDEO_FECS -f $RELAY_VIDEO_BLOCKLENGTH -t $VIDEO_FRAMETYPE -d $VIDEO_WIFI_BITRATE -y 0 relay0 > /dev/null 2>&1 &
	fi
	...
	ionice -c 1 -n 3 /root/wifibroadcast/rx -p 0 -t 6 -d 2 -b $VIDEO_BLOCKS -r $VIDEO_FECS -f $VIDEO_BLOCKLENGTH $NICS | ionice -c 1 -n 4 nice -n -10 tee >(ionice -c 1 -n 4 nice -n -10 /root/wifibroadcast_misc/ftee /root/videofifo2 > /dev/null 2>&1) >(ionice -c 1 nice -n -10 /root/wifibroadcast_misc/ftee /root/videofifo4 > /dev/null 2>&1) >(ionice -c 3 nice /root/wifibroadcast_misc/ftee /root/videofifo3 > /dev/null 2>&1) | ionice -c 1 -n 4 nice -n -10 /root/wifibroadcast_misc/ftee /root/videofifo1 > /dev/null 2>&1
...
function osdrx_function {
	...
	TELEMETRY_RX_CMD="/root/wifibroadcast/rx -p 1 -t 6 -d 2 -b $TELEMETRY_BLOCKS -r $TELEMETRY_FECS -f $TELEMETRY_BLOCKLENGTH"
    else
    ...
    killall wbc_status > /dev/null 2>&1
    ionice -c 3 nice cat /root/telemetryfifo3 >> /wbc_tmp/telemetrytmp.raw &
    ...
    while true; do
	if [ "$RELAY" == "Y" ]; then
	    ionice -c 1 -n 4 nice -n -9 cat /root/telemetryfifo4 | /root/wifibroadcast/tx -p 1 -b $RELAY_TELEMETRY_BLOCKS -r $RELAY_TELEMETRY_FECS -f $RELAY_TELEMETRY_BLOCKLENGTH -m $TELEMETRY_MIN_BLOCKLENGTH -t $TELEMETRY_FRAMETYPE -d $TELEMETRY_WIFI_BITRATE -y 0 relay0 > /dev/null 2>&1 &
	fi
...
function osdtx_function {
    ...
    while true; do
        ...
        nice cat $FC_TELEMETRY_SERIALPORT | nice /root/wifibroadcast/tx -p 1 -b $TELEMETRY_BLOCKS -r $TELEMETRY_FECS -f $TELEMETRY_BLOCKLENGTH -m $TELEMETRY_MIN_BLOCKLENGTH -t $TELEMETRY_FRAMETYPE -d $TELEMETRY_WIFI_BITRATE -y 0 $NICS
...
function mspdownlinkrx_function {
	...
	#
	#if [ "$RELAY" == "Y" ]; then
	#    ionice -c 1 -n 4 nice -n -9 cat /root/telemetryfifo4 | /root/wifibroadcast/tx -p 1 -b $RELAY_TELEMETRY_BLOCKS -r $RELAY_TELEMETRY_FECS -f $RELAY_TELEMETRY_BLOCKLENGTH -m $TELEMETRY_MIN_BLOCKLENGTH -y 0 relay0 > /dev/null 2>&1 &
	#fi
	# update NICS variable in case a NIC has been removed (exclude devices with wlanx)
	NICS=`ls /sys/class/net/ | nice grep -v eth0 | nice grep -v lo | nice grep -v usb | nice grep -v intwifi | nice grep -v wlan | nice grep -v relay | nice grep -v wifihotspot`
	nice /root/wifibroadcast/rx -p 4 -t 6 -d 2 -b $TELEMETRY_BLOCKS -r $TELEMETRY_FECS -f $TELEMETRY_BLOCKLENGTH $NICS | ionice nice /root/wifibroadcast_misc/ftee /root/mspfifo1 > /dev/null 2>&1
	echo "ERROR: MSP RX has been stopped - restarting ..."
	ps -ef | nice grep "rx -p 4" | nice grep -v grep | awk '{print $2}' | xargs kill -9
	ps -ef | nice grep "ftee /root/mspfifo1" | nice grep -v grep | awk '{print $2}' | xargs kill -9
	sleep 1
    done
}
...
function mspdownlinktx_function {
    ...
    while true; do
        ...
        nice cat $FC_MSP_SERIALPORT | nice /root/wifibroadcast/tx -p 4 -b $TELEMETRY_BLOCKS -r $TELEMETRY_FECS -f $TELEMETRY_BLOCKLENGTH -m $TELEMETRY_MIN_BLOCKLENGTH -t $TELEMETRY_FRAMETYPE -d $TELEMETRY_WIFI_BITRATE -y 0 $NICS
...
function uplinktx_function {
	...
	echo -n "NICS:"
	echo $NICS
	UPLINK_TX_CMD="nice /root/wifibroadcast/tx -p 3 -b $TELEMETRY_BLOCKS -r $TELEMETRY_FECS -f $TELEMETRY_BLOCKLENGTH -m $TELEMETRY_MIN_BLOCKLENGTH -t $TELEMETRY_FRAMETYPE -d $UPLINK_WIFI_BITRATE -y 1 $NICS"
...
function uplinkrx_function {
	...
	nice /root/wifibroadcast/rx -p 3 -t 6 -d 2 -b $TELEMETRY_BLOCKS -r $TELEMETRY_FECS -f $TELEMETRY_BLOCKLENGTH $NICS > $FC_TELEMETRY_SERIALPORT
    else # msp
	nice /root/wifibroadcast/rx -p 3 -t 6 -d 2 -b $TELEMETRY_BLOCKS -r $TELEMETRY_FECS -f $TELEMETRY_BLOCKLENGTH $NICS > $FC_MSP_SERIALPORT
    fi
}
...
function tether_check_function {
	while true; do
		...
		nice /root/wifibroadcast/rssi_forward /wifibroadcast_rx_status_0 $PHONE_IP &
...
function hotspot_check_function {
	...
	while true; do
		...
		if nice ping -I eth0 -c 1 -W 1 -n -q 192.168.1.2 > /dev/null 2>&1; then
		    ...
		    nice /root/wifibroadcast/rssi_forward /wifibroadcast_rx_status_0 $IP &
		    if [ "$FORWARD_STREAM" == "rtp" ]; then
...
case $TTY in
    ...
    ;;
    /dev/tty3) # r/c stuff
	echo "================== Dronebridge (tty3) ==========================="
	# only run rcrx if cam found
	if [ "$CAM" == "1" ]; then
	    echo "starting DroneBridge extension modules ... we are RX"
	    dronebridgerx_function
	else
	    # only run rctx if no cam found
	    if [ "$CAM" == "0" ]; then
		echo "starting DroneBridge extension modules ... we are TX"
	        dronebridgetx_function
	    fi
	fi
	#echo "DroneBridge not enabled in configfile?!"
	sleep 365d
    ;;
    /dev/tty4) # unused
	echo "================== UNUSED (tty4) ==========================="
...
#	sleep 365d
	...
	echo
	if [ "$CAM" == "0" ]; then
	    echo "Welcome to EZ-Wifibroadcast 1.5 (RX) - DroneBridge Addon - type 'rw' to make filesystems writeable"
	else
	    echo "Welcome to EZ-Wifibroadcast 1.5 (TX) - DroneBridge Addon - type 'rw' to make filesystems writeable"
	fi
    ;;

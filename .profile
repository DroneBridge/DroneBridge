...
echo
if [ "$CAM" == "0" ]; then
  echo -n "Welcome to DroneBridge v0.3.2 Beta (Ground) - "
  read -p "Press <enter> to login"
  killall osd
  rw
else
  echo -n "Welcome to DroneBridge v0.3.2 Beta (UAV) - "
  read -p "Press <enter> to login"
  rw
fi
;;
...
if [ "$CAM" == "0" ]; then
  echo "Welcome to DroneBridge v0.3.2 Beta (Ground) - type 'ro' to switch filesystems back to read-only"
  rw
else
  echo "Welcome to DroneBridge v0.3.2 Beta (UAV) - type 'ro' to switch filesystems back to read-only"
  rw
fi
;;
esac

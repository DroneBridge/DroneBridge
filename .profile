...
#	sleep 365d
    ...
    *) # all other ttys used for interactive login
	if [ "$CAM" == "0" ]; then
	    echo "Welcome to DroneBridge v0.3 Beta (Ground) - type 'ro' to switch filesystems back to read-only"
	    rw
	else
	    echo "Welcome to DroneBridge v0.3 Beta (UAV) - type 'ro' to switch filesystems back to read-only"
	    rw
	fi
    ;;
esac

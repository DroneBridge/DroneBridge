...
case $TTY in
  ...
  ;;
  /dev/tty4) # unused
  echo "================== DroneBridge v0.4 Beta (tty4) ==========================="
  if [ "$CAM" == "0" ]; then
    if [ "$RC" != "disabled" ]; then
      rssirx_function
    fi
...
echo
if [ "$CAM" == "0" ]; then
  echo -n "Welcome to DroneBridge v0.4 Beta (Ground) - "
  read -p "Press <enter> to login"
  killall osd
  rw
else
  echo -n "Welcome to DroneBridge v0.4 Beta (UAV) - "
  read -p "Press <enter> to login"
  rw
fi
;;
...
if [ "$CAM" == "0" ]; then
  echo "Welcome to DroneBridge v0.4 Beta (Ground) - type 'ro' to switch filesystems back to read-only"
  rw
else
  echo "Welcome to DroneBridge v0.4 Beta (UAV) - type 'ro' to switch filesystems back to read-only"
  rw
fi
;;
esac
